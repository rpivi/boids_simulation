...
void set_parameters(double& num_boids, double& d, double& d_s, double& s,
  ...
  void Boid::update_v(two_dim::vec const& v1, two_dim::vec const& v2, two_dim::vec const& v3) {
  ...
  // center of mass
  two_dim::vec Boid::center_mass(std::vector<Boid> const& flock, double const& d) {
    two_dim::vec x_c{0., 0.};
    int n{0};
    x_c = std::accumulate(std::begin(flock), std::end(flock), two_dim::vec{0., 0.},
